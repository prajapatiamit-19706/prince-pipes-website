...
export const MaterialsMegaMenu = ({ isOpen, onMouseEnter, onMouseLeave }) => {
  const navItem = navigationData.primary.find(item => item.id === 'materials');
  const cards = navItem?.children || [];

  const iconMap = {
    "stainless-steel": Icons.Layers,
    "duplex": Icons.Shield,
    "monel": Icons.Box,
    "nickel": Icons.BoxSelect,
  };
  ...
  return (
    <MegaMenuContainer isOpen={isOpen} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} width="320px" padding="p-4">
      <div className="flex flex-col gap-1">
        {cards.map(card => {
          const Icon = iconMap[card.id] || Icons.Layers;
          return (
