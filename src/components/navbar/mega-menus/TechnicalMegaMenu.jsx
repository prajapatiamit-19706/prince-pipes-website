...
import { FileText, Ruler, Scale } from 'lucide-react';
import navigationData from '@/data/navigation.json';

export const TechnicalMegaMenu = ({ isOpen, onMouseEnter, onMouseLeave }) => {
  const navItem = navigationData.primary.find(item => item.id === 'technical-resources');
  const cards = navItem?.children || [];

  const iconMap = {
    "standards-reference": FileText,
    "dimension-charts": Ruler,
    "weight-charts": Scale,
  };
  ...
  return (
    <MegaMenuContainer isOpen={isOpen} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} width="320px" padding="p-4">
      <div className="flex flex-col gap-1">
        {cards.map(card => {
          const Icon = iconMap[card.id] || FileText;
          return (
